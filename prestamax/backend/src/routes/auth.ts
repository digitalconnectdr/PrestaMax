...
// ── PUBLIC: Self-registration for new prestamistas ────────────────────────────
router.post('/register-tenant', async (req: Request, res: Response) => {
  try {
    ...
    if (!admin_password || admin_password.length < 8) return res.status(400).json({ error: 'La contraseña debe tener al menos 8 caracteres' });
    if (!/[A-Z]/.test(admin_password)) return res.status(400).json({ error: 'La contraseña debe contener al menos una letra mayúscula' });
    if (!/[0-9]/.test(admin_password)) return res.status(400).json({ error: 'La contraseña debe contener al menos un número' });
    if (!/[^A-Za-z0-9]/.test(admin_password)) return res.status(400).json({ error: 'La contraseña debe contener al menos un carácter especial (!@#$%^&*)' });
    ...
    if (existingUser) return res.status(400).json({ error: 'Ya existe una cuenta con este email. Inicia sesión en su lugar.' });
    ...
    // 1. Create tenant in trial status
    // Auto-assign the trial default plan if no plan_id was provided
    ...
    ).run(tenantId, company_name.trim(), slug, normalizedEmail, phone || null, currency, effectivePlanId);

    // 2. Create tenant settings
    db.prepare('INSERT OR IGNORE INTO tenant_settings (id,tenant_id) VALUES (?,?)').run(uuid(), tenantId);

    // 3. Seed default contract templates
    function seedTemplates(db: any, tid: string) {
      ...
      const contractBody = `CONTRATO DE PRÉSTAMO PERSONAL\n\nEntre {{company_name}} y el cliente {{client_name}},\nportador de la cédula {{client_id}}.\n\nMONTO:  {{amount}}\nTASA:   {{rate}}\nPLAZO:  {{term}}\nFECHA:  {{print_date}}\n\n_______________________   _______________________\nFirma del Deudor          Firma del Prestamista`;
      try {
        db.prepare('INSERT OR IGNORE INTO contract_templates (id,tenant_id,name,type,body,is_default) VALUES (?,?,?,?,?,?)')
          .run(uuid(), tid, 'Pagaré Estándar', 'general', pagareBody, 1);
        db.prepare('INSERT OR IGNORE INTO contract_templates (id,tenant_id,name,type,body,is_default) VALUES (?,?,?,?,?,?)')
          .run(uuid(), tid, 'Contrato General de Préstamo', 'general', contractBody, 0);
      } catch (_) {}
    }
    seedTemplates(db, tenantId);
    ...
    // 5. Create tenant membership as tenant_owner
    ...
    // 6. Auto-login — return token + tenants
...
router.post('/change-password', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!new_password || new_password.length < 8) return res.status(400).json({ error: 'La nueva contraseña debe tener al menos 8 caracteres' });
    if (!/[A-Z]/.test(new_password)) return res.status(400).json({ error: 'La nueva contraseña debe contener al menos una letra mayúscula' });
    if (!/[0-9]/.test(new_password)) return res.status(400).json({ error: 'La nueva contraseña debe contener al menos un número' });
    if (!/[^A-Za-z0-9]/.test(new_password)) return res.status(400).json({ error: 'La nueva contraseña debe contener al menos un carácter especial (!@#$%^&*)' });

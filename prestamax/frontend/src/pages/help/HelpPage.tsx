...
const GUIDES: Guide[] = [
  ...
  },
  {
    ...
    subtitle: 'Registra de dónde sale el dinero y a dónde entran los pagos',
  ...
  },
  {
    ...
    steps: [
      ...
      { title: 'Elige el producto', description: 'Los productos son plantillas predefinidas (ej. "Personal 3 meses") con tasas y plazos sugeridos. Puedes crear o editar productos en Configuración → Productos.' },
      { title: 'Define las condiciones', description: 'Monto, tasa de interés (%), plazo, unidad (días/semanas/meses), frecuencia de pago y tipo de amortización.', tip: 'Si no estás seguro qué tipo de amortización usar, haz clic en "¿Qué significa?" al lado del selector — verás la explicación de cada uno con ejemplo concreto.' },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: '(Opcional) Envía confirmación por WhatsApp', description: 'Si tienes WhatsApp transaccional activado, en la Bandeja aparecerá un draft de "Pago recibido" listo para enviar. Lo abres y clicas Enviar.' },
    ],
  },
  {
    id: 'crear-usuario',
  ...
  },
  {
    ...
    steps: [
      ...
      { title: '(Opcional) Personaliza la plantilla', description: 'En el tab "Plantillas" puedes crear tus propias con tu estilo. Variables: {{cliente.nombre}}, {{prestamo.monto}}, {{prestamo.proxima_cuota}}, etc. Si no creas custom, el sistema usa una por defecto en español.' },
      ...
      { title: 'Silenciar un cliente específico', description: 'Si un cliente pide no recibir mensajes (o hizo una promesa de pago y no quieres molestarlo), ve a su ficha y activa "Silenciar mensajes automáticos". No recibirá ningún draft automático hasta que lo reactives.' },
    ],
  },
]

const HelpPage: React.FC = () => {
  const [openId, setOpenId] = useState<string | null>('crear-cliente')
      ...
      <Card className="bg-amber-50 border-amber-200 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-amber-900">
            <p className="font-semibold mb-1">Sugerencia de orden para empezar</p>
            <p className="leading-relaxed">
              Para tu primer préstamo, sigue este orden:
              <strong> 1) Crea una cuenta bancaria</strong> (de dónde sale el dinero) →
              <strong> 2) Crea el cliente</strong> →
              <strong> 3) Crea el préstamo</strong>. Después ya podrás registrar pagos.
            </p>
          </div>
        </div>
      </Card>
